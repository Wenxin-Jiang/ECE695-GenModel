...
def train_and_evaluate(model, train_loader, eval_loader, criterion, optimizer, scheduler, device="cuda", epochs=10, eval_interval=10):
    ...
    for epoch in range(epochs):
        # Collect all data for the epoch
        epoch_data = []
        epoch_targets = []
        for data, target, _ in train_loader:
            data, target = data.to(device), target.to(device).long()
            epoch_data.append(data.view(data.shape[0], -1).cpu().numpy())  # Flatten and move to CPU
            epoch_targets.append(target.cpu().numpy())
        
        # Concatenate all data and targets for the epoch
        epoch_data = np.concatenate(epoch_data, axis=0)
        epoch_targets = np.concatenate(epoch_targets, axis=0)

        # Fit GMM to entire epoch data to detect and exclude outliers
        logger.info(f"Fitting GMM to detect outliers in epoch {epoch + 1}")
        gmm = GaussianMixture(n_components=40, random_state=0)  # Adjust n_components as needed
        gmm.fit(epoch_data)
        probabilities = gmm.predict_proba(epoch_data)
        non_outlier_indices = np.max(probabilities, axis=1) > 0.1  # Threshold to identify non-outliers

        # Filter out outliers based on GMM probabilities
        filtered_data = torch.from_numpy(epoch_data[non_outlier_indices]).to(device)
        filtered_target = torch.from_numpy(epoch_targets[non_outlier_indices]).to(device).long()
        ...
        if len(filtered_data) > 0:
            ...
            output = model(filtered_data)  # Forward pass
            loss = criterion(output, filtered_target)
            loss.backward()
            optimizer.step()
            total_loss = loss.item()

        # Log loss and calculate evaluation metrics
        train_losses.append(total_loss)
        logger.info(f'Epoch {epoch+1}, Loss: {total_loss:.4f}')

        if (epoch + 1) % eval_interval == 0 or epoch == epochs - 1:
            model.eval()  # Evaluation mode
            accuracy, preds, targets, features = eval(model, eval_loader, device)
    ...
    return train_losses, eval_accuracies, all_preds, all_targets, all_features
...
def eval(model, eval_loader, device="cuda"):
    ...
    with torch.no_grad():  # No need to track gradients for evaluation
        for data, target, _ in eval_loader:  # Change this line to match the expected output
            data, target = data.to(device), target.to(device).long()
            output = model(data)
            _, predicted = torch.max(output, 1) # TODO: why this is 1?
            all_preds.extend(predicted.cpu().numpy())
            all_targets.extend(target.cpu().numpy())
            all_features.extend(output.detach().cpu().numpy())
    correct = (np.array(all_preds) == np.array(all_targets)).sum()
    total = len(all_targets)
    accuracy = 100 * correct / total
    return accuracy, all_preds, all_targets, np.array(all_features)
...
def plot_confusion_matrix(preds, targets, index_to_label, label_type):
    ...
    plt.savefig(f"GMM_confusion_matrix_percentage_{label_type}.png", dpi=300)
...
def plot_loss(train_losses, epochs, lr, batch_size, label_type):
    ...
    plt.legend()
    plt.savefig(f"GMM_{label_type}_train_loss_{epochs}epochs_lr{lr}_batch{batch_size}.png")
...
def plot_accuracy(test_accuracies, epochs, lr, batch_size, label_type):
    ...
    plt.xticks(x_vals)
    plt.legend()
    plt.savefig(f"GMM_{label_type}_test_accuracy_{epochs}epochs_lr{lr}_batch{batch_size}.png")
...
def run():
    ...
    train_losses, eval_accuracies, preds, targets = train_and_evaluate(model, train_loader, eval_loader, criterion, optimizer, scheduler, epochs=epochs, eval_interval=10)

    features = extract_features_and_cluster(model, eval_loader, device)  # Extract features using your trained model
    gmm = GaussianMixture(n_components=num_classes, covariance_type='full').fit(features)
    labels_pred = gmm.predict(features)

    # Assuming you have true labels for evaluation
    print("Log-Likelihood:", gmm.score(features))  # Higher is better
    print("Silhouette Score:", silhouette_score(features, labels_pred))

    # If true labels available for purity or confusion matrix
    conf_matrix = confusion_matrix(targets, labels_pred)
...
def CV_run_GMM(label_type="arch"):
    ...
    for train_index, eval_index in kf.split(full_dataset):
        ...
        # Train and Evaluate
        train_losses, eval_accuracies, preds, targets, features = train_and_evaluate(model, train_loader, eval_loader, criterion, optimizer, scheduler, device='cuda', epochs=epochs)
        logger.success(f"Fold {fold}, Final Eval Accuracy: {eval_accuracies[-1]:.2f}%")
        cumulative_train_losses.extend(train_losses)
        cumulative_eval_accuracies.extend(eval_accuracies)

    
    
    logger.debug(f"train_losses: {cumulative_train_losses}")
    logger.debug(f"eval_accuracies: {cumulative_eval_accuracies}")
    # Calculate averages after all folds are completed
    ...
    average_train_loss = sum(cumulative_train_losses) / len(cumulative_train_losses) if cumulative_train_losses else 0
    average_eval_accuracy = sum(cumulative_eval_accuracies) / len(cumulative_eval_accuracies) if cumulative_eval_accuracies else 0

    logger.success(f"Average Train Loss: {average_train_loss:.2f}, Average Eval Accuracy: {average_eval_accuracy:.2f}%")


    logger.success(f"5-Fold Cross Validation completed with Avg Loss: {average_train_loss:.2f}, Avg Accuracy: {average_eval_accuracy:.2f}%")
...
if __name__ == "__main__":
    ...
    # run_PTMTorrent()
    # run()
    CV_run_GMM(label_type="arch")
    CV_run_GMM(label_type="model_type")
